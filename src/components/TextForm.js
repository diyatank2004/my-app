    ...
    return (
        <>
            <div className='container' style={{color:props.mode==="dark"?"white":"black"}}>
                ...
                <button disabled={text.length===0} className="btn btn-secondary mx-2 my-2" onClick={handleLcClick}>Convert To Lowercase</button>
                <button disabled={text.length===0} className="btn btn-success mx-2 my-2" onClick={removeSpaces}>Remove Extra Spaces</button>
                <button disabled={text.length===0} className="btn btn-primary mx-2 my-2" onClick={handleUpClick}>Convert To Uppercase</button>
                <button disabled={text.length===0} className="btn btn-warning mx-2 my-2" onClick={copyClipB}>Copy to Clipboard</button>
                <button disabled={text.length===0} className="btn btn-info mx-2 my-2" onClick={reverseText}>Text Reversal</button>
            ...
            <div className="container my-3">
                <h2>Text Summary</h2>
                <p>{text.split(/\s+/).filter(word => word !== "").length} words and {text.replace(/\s+/, '').length} characters</p>
                <p>{0.008 * text.split(" ").filter(word => word !== "").length} Minutes read</p>
                <h3>Preview the text</h3>
                <p style={{border:"2px solid",padding:"5px"}}>{text.length>0?text:"Nothing to preview"}</p>
            </div>
        </>
    );
}
